defmodule Day10 do
  def search(map, max_x, max_y, position) do
    ...
    set = dfs(map, max_x, max_y, position, 0, MapSet.new())
    MapSet.to_list(set) |> length()
  end

  def dfs(_, _, _, pos, 9, set) do
    MapSet.put(set, pos)
  end

  def dfs(map, max_x, max_y, {x, y}, cur, set) do
    ...
    case next_moves do
      ...
      _ ->
        next_moves
        |> Enum.reduce(set, fn coordinate, s ->
          dfs(map, max_x, max_y, coordinate, cur + 1, s)
        end)
    end
  end
