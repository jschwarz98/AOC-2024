...
map = Day18.create_grid(coordinates, 1024)
IO.puts(Day18.visualize_grid(map, max_x, max_y))

path = Day18.find_path(map, {0,0}, {max_x,max_y}, fn ({x1,y1}, {x2,y2}) -> abs(x2 - x1 + y2 - y1)  end, max_x, max_y)

map_with_path = path
|> Enum.reduce(map, fn coord, new_map ->
  Map.put(new_map, coord, :path)
end)
IO.puts("")
IO.puts("---")
IO.puts("")
IO.puts(Day18.visualize_grid(map_with_path, max_x, max_y))
IO.puts("")


IO.inspect({"found path", length(path) - 1})
